...
<jsp:include page="../header.jsp"></jsp:include>
		...
		<div class="col-md-12 wow fadeInDown" data-wow-duration="0.5s">
			<div>
				...
				<ul class="nav nav-tabs" role="tablist">
			    	...
			    	<li role="presentation" ><a href="<%=basePath %>IncomeType/incomeType_frontAdd.jsp" style="display:none;">添加收入分类</a></li>
				</ul>
			  	<!-- Tab panes -->
				    				...
				    				<table class="table table-condensed table-hover">
				    					...
				    					<% 
				    	            		...
				    	            		for(int i=0;i<incomeTypeList.size();i++) {
					    	            		...
					    	            		IncomeType incomeType = incomeTypeList.get(i); //获取到收入分类对象
 										%>
 										<tr>
 											<td><%=currentIndex %></td>
 											<td><%=incomeType.getTypeId() %></td>
 											<td><%=incomeType.getTypeName() %></td>
 											<td>
 												<a href="<%=basePath  %>IncomeType/<%=incomeType.getTypeId() %>/frontshow"><i class="fa fa-info"></i>&nbsp;查看</a>&nbsp;
 												<a href="#" onclick="incomeTypeEdit('<%=incomeType.getTypeId() %>');" style="display:none;"><i class="fa fa-pencil fa-fw"></i>编辑</a>&nbsp;
 												<a href="#" onclick="incomeTypeDelete('<%=incomeType.getTypeId() %>');" style="display:none;"><i class="fa fa-trash-o fa-fw"></i>删除</a>
 											</td> 
 										</tr>
 										<%}%>
				    				</table>
	...
	<div style="display:none;">
		...
		<form name="incomeTypeQueryForm" id="incomeTypeQueryForm" action="<%=basePath %>IncomeType/frontlist" class="mar_t15" method="post">
...
function incomeTypeDelete(typeId) {
	if(confirm("确认删除这个记录")) {
		$.ajax({
			type : "POST",
			url : basePath + "IncomeType/deletes",
			data : {
				typeIds : typeId,
			},
